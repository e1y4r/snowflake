  ...
  class extends HTMLElement {
    connectedCallback() {
      ...
      for (var j = branch_dec_count - 1; j >= 0; j--) {
        ...
        // 循环雪花的分支
        for (var i = 0; i < (j == 0 ? 1 : branch_count); i++) {
          // j == 0 时为正中间，分支数为 1
          var theta = (i * 2 * Math.PI) / branch_count - Math.PI / 2; // 雪花分支的角度
          if (isHexagon) {
            snowflakeSVG.appendChild(
              diamond || j == 0
                ? createHexagon(
      ...
      for (var i = 0; i < branch_count * 2; i++) {
        ...
        snowflakeSVG.appendChild(
          createLine(
            centerX,
            centerY,
            centerX + Math.cos(theta) * snowflake_radius,
            centerY + Math.sin(theta) * snowflake_radius,
            "#ffffff22"
          )
        );
      }
